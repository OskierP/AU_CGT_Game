...
def run_level(run):
    ...
    level_4_3_background = sprite.Sprite('data/LEVEL_3n4/assets/sprites/background/level4_3.png').load_image()
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    pygame.mixer.Sound('data/LEVEL_3n4/assets/sound/click_sound_effect.mp3').play()
                    led_flag = True
                    player_clicks.clear()
                    clicks = 0
        ...
        if level == len(level_solutions):
            time.sleep(2)
            running = False
            data.progress.save_progress.update_progress('Level_5', True)
            flags.next_lvl_4.set_flag(True)
        ...
        if clicks == len(level_solutions[led_solution_num]) - 1:
            pygame.mixer.Sound('data/LEVEL_3n4/assets/sound/success_effect.wav').play()
            level += 1
            clicks = 0
            player_clicks.clear()
            if level < 3:
